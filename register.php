...
$username_err = $email_err = $password_err =$mobile_err=$address_err= "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty(trim($_POST["password"]))) {
    ...
    if (strlen($mobile) < 8) {
      $mobile_err = "Password must contain 10 digits.";
    }
  ...
  if (empty($username_err) && empty($email_err) && empty($password_err)&& empty($mobile_err)&& empty($address_err)) {
    ...
    if ($stmt = mysqli_prepare($link, $sql)) {
      ...
      mysqli_stmt_bind_param($stmt, "sssss", $param_username,$param_password, $param_email,$param_mobile,$param_address );
